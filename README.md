...
RFlow 生成器训练 1000 个 epoch 后，几何速度场 MSE 明显收敛。训练集 epoch 平均 MSE 由 1.1524 降至 0.0187，最佳值为 0.0164；完整骨骼与假体参数条件下的验证 MSE 由 1.2614 降至 0.0162，最佳值为 0.0138（epoch 803）。训练后期验证曲线进入低位平台并伴随小幅波动，提示模型已基本学会假体 TSDF 潜变量的条件速度场，但生成质量仍需结合解码可视化、骨骼适配性和后续显式几何指标综合评价。
