...
除术前骨骼潜变量提供的逐体素空间条件外，模型进一步将假体相关参数编码为全局条件序列，并通过交叉注意力注入 UNet 的 Transformer 层。具体而言，股骨柄品牌与型号被编码为离散嵌入，结合髋臼杯外径、股骨头直径、股骨头偏心距及内衬偏心距等连续变量，共同构成一组全局条件序列。训练阶段采用无分类器引导（Classifier-free Guidance）随机丢弃策略，使模型同时学习完全条件、部分条件、完全无条件情形下生成假体，从而在不同临床规划场景下具备灵活的条件响应能力。

#### 结果评价

真实临床决策本质上是多目标约束下的折中决策。真实术后 CT 中的假体仅是可行解空间中的单一经验样本，因此不建议采用生成假体与真实术后结果之间的相似度作为评价模型生成能力的核心指标。更合理的方式是在数据集群体层面比较生成结果与真实术后结果之间的分布一致性，以评价模型是否学会真实临床决策的条件统计规律。同时，在术前骨骼条件下，有限次随机生成应能覆盖真实术后结果邻域，在指定假体参数条件变化时生成结果应能正确响应。
