...
## 条件修正流匹配（Conditional Rectified Flow Matching, RF）
...
除术前骨骼潜变量提供的逐体素空间条件外，模型进一步将假体相关参数编码为全局条件序列，并通过交叉注意力注入 UNet 的 Transformer 层。具体而言，股骨柄型号规格被编码为离散嵌入，结合髋臼杯外径、股骨头直径、股骨头偏距及内衬偏心距等连续变量，共同构成一组全局条件序列。训练阶段采用无分类器引导（Classifier-free Guidance）随机丢弃策略，使模型同时学习完全条件、部分条件、完全无条件情形下生成假体，从而在不同临床规划场景下具备灵活的条件响应能力。
...
| 术前骨骼 (x) | 假体参数 (c)  | 分布 | 模型能力 |
|:---:|:---:|:---|:---|
| 无 | 无 | $p(y)$ | 学习真实假体几何先验 |
| 有 | 无 | $p(y \mid x)$ | 生成患者匹配的假体方案 |
| 有 | 有 | $p(y \mid x,c)$ | 评价指定假体的适配性 |

除统计分布评价之外，还可采用更贴近临床医生思维习惯的评价方式，例如分析生成假体的骨皮质穿透率、髓腔有效填充率、接触初始稳定性、骨覆盖率等显式指标，并结合人类专家主观评测，综合评价生成结果的临床可行性。

## 数据集

纳入初次全髋关节置换病例共 1355 侧髋，来自 1149 名患者。其中，双侧置换 206 例，单侧置换 943 例；左侧置换 649 例，右侧置换 706 例。数据集划分为训练集 1188 例，验证集 84 例，测试集 83 例。
...
股骨柄品牌型号分布如下。验证集和测试集按股骨柄型号进行均衡抽样，主要型号各 10 例，低频型号按实际可用病例数纳入，以提高模型评价对不同假体类型的覆盖性。

| 股骨柄型号 | 全部 | 训练集 | 验证集 | 测试集 |
|:---|---:|---:|---:|---:|
| DePuy Corail | 362 | 342 | 10 | 10 |
| DePuy SUMMIT | 343 | 323 | 10 | 10 |
| Stryker Accolade TMZF | 218 | 198 | 10 | 10 |
| DePuy Tri-Lock | 183 | 163 | 10 | 10 |
| Stryker Secur-Fit | 81 | 61 | 10 | 10 |
| 未标注 | 73 | 53 | 10 | 10 |
| Wagner Cone | 53 | 33 | 10 | 10 |
| Zimmer M/L Taper | 25 | 9 | 8 | 8 |
| Zimmer Wagner SL | 9 | 3 | 3 | 3 |
| DePuy S-ROM | 6 | 2 | 2 | 2 |
| AK Medical ML-TP | 2 | 1 | 1 | 0 |

主要假体组件参数分布显示，髋臼杯外径主要集中在 48 到 54 mm，股骨头直径主要为 32 mm 与 36 mm，内衬材料以陶瓷和聚乙烯为主。

| 参数 | 分布 |
|:---|:---|
| 髋臼杯外径 | 40 mm: 1；42 mm: 3；44 mm: 34；46 mm: 111；48 mm: 306；50 mm: 293；52 mm: 263；54 mm: 202；56 mm: 109；58 mm: 20；60 mm: 11；62 mm: 2 |
| 股骨头直径 | 22 mm: 10；28 mm: 160；32 mm: 611；36 mm: 572；40 mm: 2 |
| 股骨头偏距 | -5 mm: 63；-4 mm: 67；-3.5 mm: 57；-2.7 mm: 3；-2.5 mm: 34；-2 mm: 7；0 mm: 141；1.5 mm: 376；3.5 mm: 4；4 mm: 5；5 mm: 399；8.5 mm: 134；缺失: 65 |
| 内衬偏心 | 0 mm: 51；0.25 mm: 32；0.5 mm: 52；0.75 mm: 109；1 mm: 170；1.25 mm: 224；1.5 mm: 171；1.75 mm: 145；2 mm: 69；2.25 mm: 40；2.5 mm: 27；2.75 mm: 76；3 mm: 116；3.25 mm: 58；3.5 mm: 11；3.75 mm: 3；4.5 mm: 1 |
| 内衬材料 | 陶瓷: 777；聚乙烯: 578 |
...
| 最终训练区域 | 最小尺寸 | 中位尺寸 | 最大尺寸 |
|:---|:---:|:---:|:---:|
| 术侧骨盆-股骨交集区域 | 128 × 128 × 192 | 160 × 160 × 512 | 256 × 352 × 640 |
