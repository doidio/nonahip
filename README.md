...
## 条件联合修正流匹配（Conditional Joint Rectified Flow Matching, RF）
...
在模型架构上，本研究采用由假体几何生成网络和假体参数预测头组成的解耦训练与联合推理设计。其中，假体参数预测头采用轻量级三维卷积回归网络，从当前假体几何潜变量及术前骨骼条件中预测参数语义速度场。采样阶段中，假体几何潜变量与参数语义向量在同一修正流框架下耦合演化，使模型能够在生成假体几何的同时反推对应参数。最终输出的连续参数语义向量可通过标准假体库或参数枚举进行最近邻检索，转换为股骨柄型号、规格、髋臼杯外径、股骨头直径、股骨头偏距、内衬偏心等可读参数。
...
骨性区域识别采用 TotalSegmentator 进行粗略分割，提取髋骨和股骨等解剖区域作为后续 ROI 定位依据。该步骤并不依赖分割边界作为最终监督，而是用于在大范围 CT 中自动定位术侧髋骨、股骨及其邻近假体区域，从而降低人工裁剪成本，并为后续基于真实 CT 灰度和表面重建的配准流程提供稳定初始范围。
...
在最终训练样本构建中，术前 CT 被采样为骨骼条件通道，术后 CT 则分别按髋臼侧和股骨侧变换采样为对齐参考。金属区域经形态学闭合和孔洞填充，以减少球头内部伪影变暗导致的伪空腔；随后采用 CUDA 加速的 DiffDMC 从金属体素重建假体等值面。DiffDMC 可生成封闭水密的面网格体，使后续基于连通域与空间距离的假体组件筛选能够在自动排除内固定钢板、钢圈等无关金属的同时，充分保留可能因金属伪影局部断裂而被错误分离的有效假体实体。由于假体 TSDF 的符号判定依赖闭合实体表面，水密网格也是从术后金属分割稳定生成髋臼侧和股骨侧 TSDF 通道的关键前提。

#### 数据自动化处理与质控

为支撑千例级 CT 数据的自动化处理，数据处理管线中的网格最近点查询、金属距离场计算、ROI 重采样、等值面重建、假体 TSDF 采样和复核图像生成均采用 GPU 并行加速。其中，NVIDIA Warp 用于加速 ICP 迭代、金属伪影避让采样、体素重采样和 TSDF 采样，并通过缓存网格空间索引减少重复开销；DiffDMC 用于加速金属体素的水密等值面重建。上述优化使配准变换、训练图像、假体 TSDF 和可视化复核图像能够在统一流程中批量生成。自动化处理结果最终经人工可视化审查确认，以提高数据集构建的一致性、可复现性和质量可控性。
