...
在最终训练样本构建中，术前 CT 被采样为骨骼条件通道，术后 CT 则分别按髋臼侧和股骨侧变换采样为对齐参考。金属区域经形态学闭合和孔洞填充，以减少球头内部伪影变暗导致的伪空腔；随后采用 CUDA 加速的 [DiffDMC](https://github.com/SarahWeiii/diso) 从金属体素重建假体等值面。DiffDMC 可生成封闭水密的面网格体，使后续基于连通域与空间距离的假体组件筛选能够在自动排除内固定钢板、钢圈等无关金属的同时，充分保留可能因金属伪影局部断裂而被错误分离的有效假体实体。由于假体 TSDF 的符号判定依赖闭合实体表面，水密网格也是从术后金属分割稳定生成髋臼侧和股骨侧 TSDF 通道的关键前提。

#### 数据自动化处理与质控

为支撑千例级 CT 数据的自动化处理，数据处理管线针对网格最近点查询、金属距离场计算、ROI 重采样、等值面重建、假体 TSDF 采样和复核图像生成等关键环节进行了并行化优化。其中，基于 [NVIDIA Warp](https://github.com/nvidia/warp) 定制实现了网格查询、距离场计算、体素重采样和 TSDF 采样等核函数，用于加速 ICP 迭代、金属伪影避让采样和训练样本生成，并通过缓存网格空间索引减少重复开销；DiffDMC 用于加速金属体素的水密等值面重建。上述优化使配准变换、训练图像、假体 TSDF 和可视化复核图像能够在统一流程中批量生成。自动化处理结果最终经人工可视化审查确认，以提高数据集构建的一致性、可复现性和质量可控性。
