...
为支撑千例级 CT 数据的自动化处理，数据处理管线中的网格最近点查询、金属距离场计算、ROI 重采样、等值面重建、假体 TSDF 采样和复核图像生成均采用 GPU 并行加速。其中，[NVIDIA Warp](https://github.com/nvidia/warp) 用于加速 ICP 迭代、金属伪影避让采样、体素重采样和 TSDF 采样，并通过缓存网格空间索引减少重复开销；[DiffDMC](https://github.com/SarahWeiii/diso) 用于加速金属体素的水密等值面重建。上述优化使配准变换、训练图像、假体 TSDF 和可视化复核图像能够在统一流程中批量生成。自动化处理结果最终经人工可视化审查确认，以提高数据集构建的一致性、可复现性和质量可控性。
