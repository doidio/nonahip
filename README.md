...
![Autoencoder validation L1 loss](doc/vae_validation_l1.svg)

![Autoencoder validation PSNR](doc/vae_validation_psnr.svg)

![Autoencoder validation SSIM](doc/vae_validation_ssim.svg)
...
验证曲线显示，两类自编码器均在较低重建误差下取得较高的重建质量。骨骼自编码器训练更平稳，验证 L1 最低为 0.007946，PSNR 最高为 36.77 dB，SSIM 最高为 0.9848；假体 TSDF 自编码器在最佳 checkpoint 附近达到更低 L1 与更高 PSNR/SSIM，分别为 0.003313、47.23 dB 和 0.9973。后续训练中假体自编码器验证指标出现回落，因此采用验证最优权重作为潜空间生成模型的假体解码器。

两类自编码器潜变量接近零均值，缩放因子位于 [0.1, 10] 常见合理区间，能够有效实现高分辨率三维图像的潜空间压缩。其中，假体 TSDF 的缩放因子高于骨骼图像，可能与 TSDF 表征相对稀疏、有效几何信息集中于假体表面附近有关。
...
## 条件修正流匹配（Conditional Rectified Flow Matching）
...
flowchart TB
  ...
  %% 多模态输入
  z_concat["多通道拼接潜变量 y | x"]

  %% 网络模块
  bone_encoder["骨骼潜空间编码器 AutoencoderKL Encoder"]
  impl_encoder["假体潜空间编码器 AutoencoderKL Encoder"]
  geom_flow["假体流场预测网络 3D UNet"]
  param_embedder["结构化假体条件编码器 Embedding/Linear"]
  transformer["深层 Transformer"]

  %% 编码与加噪
  pre_ct -->|骨骼提取| bone_encoder -->|压缩编码| z_bone
  post_ct -->|假体几何提取| impl_encoder -->|压缩编码| z_impl_t
  eps_y --->|加噪| z_impl_t

  param_record -->|字段解析| param_embedder
  param_embedder --->|全局条件注入| transformer

  %% 条件注入
  z_bone -->|空间条件注入| z_concat
  z_impl_t -->|通道拼接| z_concat
  transformer -->|交叉注意力| geom_flow

  z_concat -->|特征输入| geom_flow

  %% 流场积分与解码评价
  flow_solver{"修正流积分器"}
  loop_cont["继续循环"]
  z_impl_0["假体几何潜变量 y_0"]
  impl_decoder["假体潜空间解码器 AutoencoderKL Decoder"]
  eval_pre_ct["术前 CT 影像"]
  geom_eval["验证假体几何生成能力"]

  geom_flow --->|假体几何流场 y_v| flow_solver
  flow_solver -->|t &gt; 0| loop_cont
  flow_solver -->|t = 0| z_impl_0
  z_impl_0 -->|重建解码| impl_decoder -->|可视化| geom_eval
  eval_pre_ct -->|骨骼适配性对照| geom_eval
```
...
假体参数不同于术前骨骼的空间约束，主要提供型号、规格及组件参数层面的全局结构先验，因此通过深层 Transformer 注入，使假体几何中杯、柄、头等不同细节特征在生成过程中选择性注意到相应参数约束。训练过程中对骨骼条件和假体参数条件均采用无分类器条件丢弃（Classifier-free Condition Dropout），使同一模型能够覆盖无条件、仅骨骼条件、仅假体参数条件以及骨骼与部分参数联合条件等生成场景。

训练阶段按照流匹配范式对假体潜变量加噪并学习速度场，验证阶段从噪声潜变量积分采样，经潜空间解码器重建为假体几何图像，并与术前骨骼图像在同一空间进行可视化对照，以评价生成结果的几何完整性与骨骼适配性。当前一阶段 1-RF 采样通常需 3 至 5 步即可完成，进一步实现单步生成需引入二阶段 2-RF 整流训练，但预期收益有限，现有推理效率已能满足临床规划需求。

#### 结果评价

![RFlow geometry velocity MSE](doc/rflow_geometry_mse.svg)

RFlow 生成器训练 1000 个 epoch 后，几何速度场 MSE 明显收敛。训练集 epoch 平均 MSE 由 1.1524 降至 0.0187，最佳值为 0.0164；完整骨骼与假体参数条件下的验证 MSE 由 1.2614 降至 0.0162，最佳值为 0.0138（epoch 803）。训练采用较大的有效 batch size 并结合指数移动平均（exponential moving average, EMA）权重，以降低单例动态尺寸训练带来的梯度波动并稳定采样质量。曲线显示损失可在早期快速降至较低水平，但流匹配生成器的几何细节仍需要长时间持续优化；从验证可视化结果看，髋臼杯、股骨头和股骨柄尾端等结构会随训练逐步成形，边缘形态最终趋于清晰、坚硬和稳定。
...
flowchart TB
  ...
  real_latent --> metal_cls
  gen_param --> rflow_gen --> gen_latent --> metal_cls
...
假体参数判别器作为假体几何生成器之后的独立评价模块，直接从假体几何潜变量预测股骨柄型号、股骨柄规格、髋臼杯外径、股骨头直径、股骨头偏距和内衬偏心等结构化参数。其主体为轻量级三维卷积网络，通过共享特征骨干和多头输出同时处理离散型号、型号内规格和有序数值参数；缺失标注字段仅参与有效标签对应的损失和指标计算。

损失函数按股骨柄型号、股骨柄规格和四类组件参数分组计算。股骨柄型号采用交叉熵监督，股骨柄规格在真实型号约束下计算，以避免跨型号规格混淆；髋臼杯外径、股骨头直径、股骨头偏距和内衬偏心按标准枚举值构建有序软标签。最佳模型依据均衡验证指标保存，该指标等权综合型号、规格和组件参数三类结果，更适合评价多头判别器在临床关键参数上的整体可靠性。
...
| 数据域 | 几何来源 | 参数条件 | 主要作用 | balanced score 最佳值 |
|:---|:---|:---|:---|---:|
| 真实域 | 术后 CT 真实假体潜变量 | 临床真实记录 | 真实分布参照与跨域验证 | 0.4702 |
| 生成域 | RFlow 参数条件生成假体潜变量 | 真实数据中出现过的参数组合 | 主训练域与主评价域，体现生成式数据扩增价值 | 0.5358 |
| 扩展域 | RFlow 参数条件生成假体潜变量 | 手册允许范围内采样组合 | 参数空间外推压力测试 | 0.2278 |
